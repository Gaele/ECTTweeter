...
public class LevelManager extends AbstractManager {
	...
	public void learn(final ArrayList<ArrayList<Tweet>> datas, final double k,
		...
		this.classifieur.learn(this, datas, k, verbose);
	}
	...
	public ArrayList<ArrayList<Tweet>> work(final ArrayList<Tweet> dataTest,
			final boolean verbose) {
		return this.classifieur.work(dataTest, true);
	}
	...
	public double check(final ArrayList<ArrayList<Tweet>> res,
			final boolean verbose) {
		final double accuracy = this.classifieur.check(res, verbose);
		if (verbose) {
			this.classifieur.calculateAndDisplayConfusionMatrix(res);
		}
		return accuracy;
	}
