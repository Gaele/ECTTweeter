...
public abstract class Classifier {
	...
	public void learn(final AbstractManager man,
		...
		nextLocalId = 0;
		final ArrayList<ArrayList<Tweet>> datas = preTraitement(man,
				globalDatas, localDictionary);
	...
	public double check(final ArrayList<ArrayList<Tweet>> res,
		...
		for (final ArrayList<Tweet> classe : res) {
			for (final Tweet t : classe) {
				if (nbClasse == toDerivatedClasses[t.getPolarit()]) {
					okStats[nbClasse]++;
					ok++;
				}
				total++;
				totStats[toDerivatedClasses[t.getPolarit()]]++;
				nbInClasse[nbClasse]++;
			}
			nbClasse++;
		}
	...
	public void calculateAndDisplayConfusionMatrix(
		...
		for (final ArrayList<Tweet> classe : evaluationTweets) {
			for (final Tweet tweet : classe) {
				final int real = toDerivatedClasses[tweet.getPolarit()];
