 ...
 * treatment and local performance analysis.
 * 
 * @author vincent
 * 
 */
public abstract class Classifier {
	...
	public Classifier() {
		this.localDictionary = new HashMap<Integer, Integer>();
	}

	/**
	 * Weather this tweet can be used to learn by this classifier. Some
	 * classifiers can learn on some special student level or tongue
	 * 
	 * @param t
	 *            the Tweet
	...
	/**
	 * Initialises the local dictionary and transform final/global classes into
	 * derived/local classes.
	 * 
	 * @param man
	 *            the manager
	 * @param datas
	 *            the global datas (orded by final classes)
	 ...
	 * 
	 * @param man
	 *            the manager using this classifier
	 * @param globalDatas
	 *            the global datas (ordered by final classes)
	 * @param k
	 *            the k parameter
	 * @param verbose
	 *            prints datas if true
	 */
	public void learn(final AbstractManager man,
			final ArrayList<ArrayList<Tweet>> globalDatas, final double k,
		...
		this.py = new double[this.NB_CLASSES_DERIVEES];
		this.alpha = new double[this.NB_CLASSES_DERIVEES];
		this.byw = new double[this.NB_CLASSES_DERIVEES][this.localDictionary
				.size()];
		// Calculates Py
		this.calculatePy(datas);
		final Long endPy = System.nanoTime();
		if (verbose) {
			System.out.println(">>> Time for Py: " + (endPy - endLocalDico)
					/ 1000000000 + " sec");
		}
		// Calculates Byw
		this.calculateByw(man, datas, k);
		final Long endByw = System.nanoTime();
		if (verbose) {
			System.out.println(">>> Time for Byw: " + (endByw - endPy)
					/ 1000000000 + " sec");
		}
		// Calculates A
		this.calculateA(man);
		if (verbose) {
			System.out.println(">>> Time for A: "
					+ (System.nanoTime() - endByw) / 1000000000 + " sec");
		}
	}

	/**
	 * Integer to Natural, gives the string representation of a derived/local
	 * classe code.
	 * 
	 * @param polarite
	 *            the code of the derived/local classe
	...
	private void calculatePy(final ArrayList<ArrayList<Tweet>> datas) {
		...
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
	 ...
	 *            the k parameter
	 */
	private void calculateByw(final AbstractManager man,
			final ArrayList<ArrayList<Tweet>> datas, final double k) {
		// nb tweets in class i
		final double[] dy = new double[this.NB_CLASSES_DERIVEES];
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
			dy[i] = datas.get(i).size();
		}

		// content of hash table : (class number, (word number in dictionary, nb
		...
		// map initialization
		int classNb;
		for (classNb = 0; classNb < this.NB_CLASSES_DERIVEES; classNb++) {
			occurrences.put(classNb, new HashMap<Integer, Integer>());
		}
		classNb = 0; // must start from the same number as in map initialization
						// above
		...
		for (final ArrayList<Tweet> classe : datas) {
			...
			for (final Tweet tweet : classe) {
				...
				for (final Integer wordNb : tweet.getWords()) {
					final Integer wordOcc = occurrences.get(classNb)
							.get(wordNb);
					if (wordOcc == null) { // first time to see the word in this
											// class
		...
		for (classNb = 0; classNb < this.NB_CLASSES_DERIVEES; classNb++) {
			for (final int wordNb : this.localDictionary.values()) {

				final Integer wordOcc = occurrences.get(classNb).get(wordNb);
				if (wordOcc == null) {
					this.byw[classNb][wordNb] = k / dy[classNb];
				} else {
					this.byw[classNb][wordNb] = Math.max(k,
							Math.min(wordOcc, dy[classNb] - k))
							/ dy[classNb];
				}
			}
		}
	...
	private void calculateA(final AbstractManager man) {
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
			...
			this.alpha[i] = 0;
			for (final Integer w : this.localDictionary.values()) {
	...
	private int calculatePxy2(final Tweet t) {
		...
		// for all class
		for (int i = 0; i <= this.NB_CLASSES_DERIVEES - 1; i++) {
			logPXY = this.alpha[i];
	 ...
	 *            the tweets
	 * @param checkUsable
	 *            if true, we use only the tweets which are
	 *            {@link #isUsable(Tweet)}. CheckUsable is true for local
	 *            analysis, false for global analysis.
	 * @return datas with their local class : the posotion of each list of tweet
	 *         is a local class' code.
	 */
	public ArrayList<ArrayList<Tweet>> work(final ArrayList<Tweet> tweets,
			final boolean checkUsable) {
		...
		for (final Tweet t : tweets) {
			if (checkUsable && !this.isUsable(t)) {
				continue;
			}
	 ...
	 * 
	 * @param res
	 *            the calculated datas
	 * @param verbose
	 *            prints infos if true
	...
	public double check(final ArrayList<ArrayList<Tweet>> res,
		...
		int nbClasse = 0;
		double total = 0, ok = 0;
		final int okStats[] = new int[this.NB_CLASSES_DERIVEES];
		final int totStats[] = new int[this.NB_CLASSES_DERIVEES];
		final int nbInClasse[] = new int[this.NB_CLASSES_DERIVEES];
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
			okStats[i] = 0;
			totStats[i] = 0;
			nbInClasse[i] = 0;
		...
		if (verbose) {
			...
			for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
				...
				System.out.println("err Classe " + this.itn(i) + " ("
						+ nbInClasse[i] + "): " + (totStats[i] - okStats[i])
						+ "/" + totStats[i] + " => "
						+ (totStats[i] - okStats[i]) * 100 / totStats[i] + "%");
			}
			// words known and unknown. can't be used for pipe-lined work since
			// the final checking classifier don't have any dictionary.
			// final int nbWordsKnown = byw[0].length;
			// final int totNbWords = localDictionary.size();
			// System.out.println("nb words: " + totNbWords);
			// System.out.println("unknown words: " + ((double)totNbWords -
			// nbWordsKnown) / totNbWords * 100);
		}
	...
	public void displayByw() {
		...
		for (int i = 0; i < this.localDictionary.size(); i++) {
			System.out.print("\t" + i);
		}
		System.out.println();
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
			// display class
			System.out.println(this.itn(i));
			for (int j = 0; j < this.localDictionary.size(); j++) {
	 ...
	 *            the tweets to evaluate, sorted by derived/local class
	 */
	public void calculateAndDisplayConfusionMatrix(
			final ArrayList<ArrayList<Tweet>> evaluationTweets) {
		final Integer[][] confusionMatrix = new Integer[this.NB_CLASSES_DERIVEES][this.NB_CLASSES_DERIVEES];
		for (int i = 0; i < this.NB_CLASSES_DERIVEES; i++) {
	...
	public int getNB_CLASSES_DERIVEES() {
		return this.NB_CLASSES_DERIVEES;
	}
